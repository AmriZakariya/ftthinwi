...
class AnnulationFormBloc extends FormBloc<String, String> {
  ...
  AnnulationFormBloc() : super(isLoading: true) {
  ...
  Future<String> callWsAddMobile(Map<String, dynamic> formDateValues) async {
    ...
    try {
      ...
      apiRespon = await dio.post("${Tools.baseUrl}/demandes/annuler_mobile",
          data: formData,
          options: Options(
...
class AnnulationForm extends StatefulWidget {
  @override
  _AnnulationFormState createState() => _AnnulationFormState();
}

class _AnnulationFormState extends State<AnnulationForm>
  ...
  FormBlocStep _step1(AnnulationFormBloc formBloc) {
    return FormBlocStep(
      title: Text('Annulation de la demande'),
      content: Column(
        children: <Widget>[
